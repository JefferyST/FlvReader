...
int CFlvParser::ParseFile()
{
	...
	while (curPos<m_uiFileLen-4)//Remove the last four ending flags£º00 00 00 10
	{
		...
		case TAG_TYPE_AUDIO:
		{
			...
			if (!m_bIsAdtsHeadInit && A_CODEC_ID_AAC == m_mediaInfo.audio_codec_id)
			{
				...
				if (0x00 == aacPacketType)
				{
					...
					m_mediaInfo.AacHeader[3] |= (0 << 5);                      //original£º0                               1bit
					m_mediaInfo.AacHeader[3] |= (0 << 4);                      //home£º0                                   1bit
					m_mediaInfo.AacHeader[3] |= (0 << 3);                      //copyright id bit£º0                       1bit  
					m_mediaInfo.AacHeader[3] |= (0 << 2);                      //copyright id start£º0                     1bit
					//m_mediaInfo.AacHeader[3] |= ((m_mediaInfo. & 0x1800) >> 11); //frame length£ºvalue,                  high 2bits 
			...
			{
				tmpPacket.size = tagheader_datasize - 1 - 1; //minus 1 byte£ºAACPacketType
				//skip 1 byte£ºAACPacketType 
			...
			//cout<<c++<<" Audio£º"<< tagheader_datasize<<" "<<tmpPacket.offset<<endl;
		}
		break;
		case TAG_TYPE_VIDEO:
		{
...
uint32_t CFlvParser::ReadAvFrame(DataPacket &packet, int64_t index)
{
	...
	else if (TYPE_AUDIO == packet.stream_type)
	{
		...
		if (A_CODEC_ID_AAC == m_mediaInfo.audio_codec_id)
		{
			...
			m_mediaInfo.AacHeader[3] |= ((tmpLen & 0x1800) >> 11);           //frame length£ºtmpLen      high 2bits
			m_mediaInfo.AacHeader[4] = (uint8_t)((tmpLen & 0x7f8) >> 3);     //frame length£ºtmpLen      middle 8bits 
			m_mediaInfo.AacHeader[5] = (uint8_t)((tmpLen & 0x7) << 5);       //frame length£ºtmpLen      low 3bits  
			m_mediaInfo.AacHeader[5] |= 0x1f;                                //buffer fullness£º0x7ff    high 5bits
